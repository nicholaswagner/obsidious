...
const FileExplorer = () => {
    ...
    const pathIds = latestLocation.pathname
        .replace(basepath, '')
        .split('/')
        .map((path) => {
            if (path === '') return
        ...
        <StyledTreeView
            ...
            onExpandedItemsChange={(_event, ids) =>
                setExpandedItems((prev) => [
                    ...prev.filter((ids) => !ids.includes(ids)),
                    ...ids,
                ])
            }
            ...
            onItemClick={(_, itemId) => {
                ...
                if (
                    file.extension &&
                    file.extension in ObsidiousVaultImageFiletypes
                ) {
